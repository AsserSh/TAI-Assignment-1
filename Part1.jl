...
# ╔═╡ 3ff2ec20-277f-11f0-0021-4bb8910746cf
using PlutoUI

# ╔═╡ 6374066a-a5c5-4dee-92d8-52633c61889d
using Markdown

# ╔═╡ fd325f28-1771-4c92-b576-01889d50b025
using InteractiveUtils

# ╔═╡ 914b9b42-a5ec-459e-a14a-b0ffa38528c1
using CSV

# ╔═╡ 943af57b-78c7-4281-93e6-8ea46523b6fe
using DataFrames

# ╔═╡ 5e50cd73-b39a-4bd7-9985-842600671d92
using MLJ

# ╔═╡ b8f5f9e3-61be-4e43-8f52-c0c855736a8f
using Statistics

# ╔═╡ e9eb564c-46b3-439d-bf6f-deb2c4c772c7
using StatsBase

# ╔═╡ 764ae419-793e-41ba-8e7f-2017e1005cbb
using MLJClusteringInterface

# ╔═╡ c8363987-5aa6-4225-bb24-b9f664449996
using GaussianMixtures

# ╔═╡ 9525288a-8812-4155-9145-f2ccd6bd3533
using MultivariateStats: fit, transform

# ╔═╡ 017dcc0f-d958-4b92-bd29-54b2a20cd064
using Distances

# ╔═╡ 6fa68188-5be9-401c-9c54-3c065a3949aa
using Clustering

# ╔═╡ be10f3be-1255-4e07-b554-1d29f4a6b2b1
using MLJMultivariateStatsInterface

# ╔═╡ 9144985a-1c11-4af8-834f-ceface474ade
using StatsPlots

# ╔═╡ ab0011ad-e5e0-4f56-909d-8571d3c0895a
using AlgebraOfGraphics, CairoMakie

# ╔═╡ 89d06373-d304-4c11-a4af-94f9b2676745
using BetaML

# ╔═╡ 947d0036-5a15-45e5-8c9a-4d6de76e140c
PlutoUI.TableOfContents()

# ╔═╡ 67021052-f96f-4e48-a855-5467c0dd12e0
md"## About the dataset"

# ╔═╡ f6cb90e8-36fe-4995-a95b-4df1ed5b2989
md"""The dataset contains drug-induced autoimmune, from a file called dia.csv"""

# ╔═╡ d67e25e3-3a38-40ec-b54d-7e3893ab4073
md"## Cleaning Dataset"

# ╔═╡ 37cc4163-3102-46d7-b0b2-f0ebfc9f4395
data_dia = CSV.read("C:/Users/Asser/Documents/NUST/AI/Assignment_2/dia.csv", DataFrame)

# ╔═╡ 6c01143b-cbc0-471c-a657-adb6d8e2518f
select!(data_dia, Not(:SMILES))

# ╔═╡ 91a39929-fe60-4503-aa07-96198bd3ea0f
data_cpy = deepcopy(data_dia)

# ╔═╡ eb49a5cd-379b-4472-9c6e-d144af6a3e60
data_clean = data_cpy[:, [!all(==(0), col) for col in eachcol(data_cpy)]]

# ╔═╡ 2116a2c3-636f-490b-a9cd-057277bd7eb8
schema(data_clean) |> DataFrames.DataFrame

# ╔═╡ a6631c7d-ed35-4685-9a60-530cbd420c92
md"""The cell below will standardize the continuous columns and hot-encode the categorical ones"""

# ╔═╡ cc43ec02-cb67-43a4-80a0-9df8d2ee825d
coerce!(data_clean, Count => Continuous)

# ╔═╡ ded43d72-8968-44c7-a3c6-fcb5df91e70d
transformed_match = machine(Standardizer(), data_clean) |> MLJ.fit!

# ╔═╡ 6d6ed309-8bba-452a-9b83-ea33bdbb3bb4
ind_var = MLJ.transform(transformed_match, data_clean)

# ╔═╡ 104701f3-b787-44b2-aa97-1437c26c08fd
md"## Building Model"

# ╔═╡ e4e3cab2-c5b9-4e09-8ae3-96c78d17940a
md"### K-Means Clustering"

# ╔═╡ e5453ad9-a468-4af9-9508-3664dab9863e
KMeans = @load KMeans pkg=Clustering

# ╔═╡ 362423da-4a8d-46ca-ad43-ac4dcbbce3b5


# ╔═╡ a94ae7a8-2cd3-4234-b2cc-a9bbc3d00d14
obj_model = Standardizer() |> MLJ.OneHotEncoder() |> KMeans(k=2)

# ╔═╡ bcf8e5c0-11df-4101-8577-2bbac2237f4a
obj_mach = machine(obj_model, data_clean)

# ╔═╡ 1089dddf-d74b-485e-a0b0-7a8c3700119d


# ╔═╡ 34706707-55a0-46de-8c76-b134e94c4b4c
MLJ.fit!(obj_mach)

# ╔═╡ ca47d34a-2042-49a5-83be-562560bec2f8
rpt = report(obj_mach)

# ╔═╡ d6b3332f-c7ad-4287-acb0-fe0ae5e93fde
fitted_params(obj_mach).k_means.centers'

# ╔═╡ c650a3fb-6bc3-4db2-afe0-55d2e0ed4619
ind_mat = Matrix(ind_var)

# ╔═╡ ab66e8d5-db01-4c08-b466-3814bc93dabc
km_sil = silhouettes(rpt.k_means.assignments, ind_mat'; metric=SqEuclidean(), batch_size=256)

# ╔═╡ 701ac691-d1e7-472b-bec9-a5f83294e1cf
mean(km_sil)

# ╔═╡ e399a96d-bb1c-4bec-8aff-86b1ec5f5160
md"### Agglomerative Clustering"

# ╔═╡ 8e9c0893-53c1-4e1c-a95f-b7d8ac2f6396
HierarchicalClustering = @load HierarchicalClustering

# ╔═╡ 506995b5-f0e0-4cd4-ae24-f1d0102fb7f9
obj_model2 = HierarchicalClustering(linkage = :complete, metric=SqEuclidean())

# ╔═╡ 884efc23-2ef3-42c8-b0e1-52a317ab3382
PCA = @load PCA pkg = MultivariateStats

# ╔═╡ cd105075-b674-4e1f-9b52-3c34120783de
data_pca_model = PCA(maxoutdim=2,variance_ratio=1)

# ╔═╡ bdb912f7-d0d9-4264-9a1a-4283d98dda25
data_pca_mach = machine(data_pca_model, ind_var)

# ╔═╡ a2e12399-52c0-4452-b4db-a8a1e078d636
MLJ.fit!(data_pca_mach)

# ╔═╡ 99c4541b-3536-4508-87c5-4d6148f224f0
ind_pca = MLJ.transform(data_pca_mach, ind_var)

# ╔═╡ 6f6d75c8-5ed5-46c0-b01f-94e7e81dc96c
obj_mach2 = machine(obj_model2)

# ╔═╡ f67b4486-5b0f-4d82-a620-99235f885172
grp_part1 = MLJ.predict(obj_mach2, ind_pca[1:500,:])

# ╔═╡ 1f453aa9-0fde-4638-ab49-9220c0f621b0
levels(grp_part1)

# ╔═╡ 1df0bd13-8f36-43f7-8acf-db6e343b11ea
rpt2 = report(obj_mach2)

# ╔═╡ 849f92ce-576e-4b21-9003-0316cb517101
StatsPlots.plot(rpt2.dendrogram)

# ╔═╡ 54cafdba-2f26-430b-8d9c-02e94dc76dc4
report(obj_mach2).cutter(h = 30)

# ╔═╡ 59ac9dbb-8fcf-4bfb-aea8-edb180f3971e
data_pca_part1 = ind_pca[1:500, :]

# ╔═╡ 5c7d1e6c-a7b4-4439-a126-dff1991ef838
data_pca_part1_mat = Matrix(data_pca_part1)

# ╔═╡ db22825d-f8cd-4056-9807-e312e87df9df
hc_sils = silhouettes(convert(Vector{Int}, grp_part1), data_pca_part1_mat'; metric=SqEuclidean(), batch_size=256)

# ╔═╡ 11e1b553-1945-42ee-be6e-8f88d99ece98
mean(hc_sils)

# ╔═╡ 04dc40d4-3c27-4e05-928b-68bf8dc15f7b
rpt2.cutter(h=30, k=3)

# ╔═╡ 602762d3-b34b-45cf-925f-2d392fbc27fa
data_pca_mat = Matrix(ind_pca)

# ╔═╡ d9c3010c-7157-48b4-805a-9b624fb5e0a9
grp_part1v1 = hclust(Distances.pairwise(Euclidean(), data_pca_mat, data_pca_mat), linkage=:complete)

# ╔═╡ 41098bbd-d053-44b3-80c7-86962391abbe
data(ind_pca) * AlgebraOfGraphics.density() * mapping(:x1) |> draw()

# ╔═╡ ea1f4baf-4a44-4faf-bfc5-652c675f5ddc
data(ind_pca) * mapping(:x1, :x2) * visual(Scatter) |> draw()

# ╔═╡ 49b0f1c9-f29c-4022-b2df-73a548a98af3
md"### Gaussian Mixture Model"

# ╔═╡ 9dbc6441-3ee7-4658-b7b3-8433a7b9b3d6
GMM   = @load GaussianMixtureClusterer verbosity=0

# ╔═╡ 7a455ee8-771c-4496-b26a-6542e2217a03
obj_model4 = GMM(n_classes=2)

# ╔═╡ bf42acb2-c0db-4adf-91a2-e0be814581fe
obj_mach4 = machine(obj_model4, ind_pca)

# ╔═╡ dde40e07-b19a-4d20-aaf7-67a0d6a6dff2
MLJ.fit!(obj_mach4)

# ╔═╡ f3dd41d1-4a48-4934-bd11-e3d8c46e8f73
predict4 = MLJ.predict(obj_mach4, ind_pca)

# ╔═╡ 4e47aa34-f8a3-4943-87ed-74a01d4ad3da
md"### Conclusion "
...
manifest_format = "2.0"
project_hash = "cc97121d71653686b935852e8b249a99633635b4"
...
git-tree-sha1 = "0378c261e8351210bc7a1ec16f0320f32d2847a3"
...
git-tree-sha1 = "8333d4c6ba83914d4204fa9fcf2c0a5bc73fe99c"
...
[[deps.GaussianMixtures]]
deps = ["Arpack", "Clustering", "Compat", "DelimitedFiles", "Distributed", "Distributions", "FileIO", "JLD2", "LinearAlgebra", "Logging", "PDMats", "Printf", "Random", "ScikitLearnBase", "SpecialFunctions", "Statistics", "StatsBase"]
git-tree-sha1 = "23a5bb0ff490e13c8a6a26d74dec87e6698e81ff"
...
deps = ["Clustering", "Distances", "LinearAlgebra", "MLJModelInterface", "StatsBase"]
git-tree-sha1 = "016fc1479a9e8816db16e82605e2f408be22f1c2"
